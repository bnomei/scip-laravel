...
final class DocIndexer
{
    ...
    private function def(
        ...
        $this->occurrences[] = new Occurrence([
            ...
            'syntax_kind'  => $kind,
            'enclosing_range' => $this->enclosingRange($pos, $posNode),
        ]);
    }
    ...
    private function ref(
        ...
        if (!str_starts_with($symbol, 'local ')) {
            ...
            if ($this->composer->isDependency($ident)) {
                $this->extSymbols[$symbol] = new SymbolInformation([
                    'symbol' => $symbol,
                    'display_name' => $this->externalDisplayName($symbol),
                    'kind' => $this->externalKind($symbol),
                    'documentation' => $this->externalDocumentation($symbol),
                ]);
            }
        }
        ...
        $this->occurrences[] = new Occurrence([
            'range'        => $pos->pos($posNode),
            'symbol'       => $symbol,
            'symbol_roles' => $role === SymbolRole::UnspecifiedSymbolRole ? SymbolRole::ReadAccess : $role,
            'syntax_kind'  => $kind,
            'enclosing_range' => $this->enclosingRange($pos, $posNode),
        ]);
    }
     ...
     * @return array<int>
     */
    private function enclosingRange(PosResolver $pos, Node $node): array
    {
        $parent = $node->getAttribute('parent');

        while ($parent instanceof Node) {
            if ($parent instanceof ClassLike || $parent instanceof FunctionLike) {
                return $pos->pos($parent);
            }
    ...
    private function externalDisplayName(string $symbol): string
    {
        $descriptor = $this->externalDescriptor($symbol);
        ...
        if (str_ends_with($descriptor, '#')) {
            return basename(str_replace('/', DIRECTORY_SEPARATOR, substr($descriptor, 0, -1)));
        }

        if (str_ends_with($descriptor, '().')) {
            return basename(str_replace('/', DIRECTORY_SEPARATOR, substr($descriptor, 0, -3))) . '()';
        }
    ...
    private function externalKind(string $symbol): int
    {
        $descriptor = $this->externalDescriptor($symbol);
        ...
        return match (true) {
            preg_match('/#\\$[^.#]+\\.$/', $descriptor) === 1 => Kind::Property,
            preg_match('/#[^.#()]+\\(\\)\\.$/', $descriptor) === 1 => Kind::Method,
            preg_match('/#[^.#()]+\\.$/', $descriptor) === 1 => Kind::Constant,
            str_ends_with($descriptor, '#') => Kind::PBClass,
            str_ends_with($descriptor, '().') => Kind::PBFunction,
            str_ends_with($descriptor, '.') => Kind::Constant,
            default => Kind::Object,
        };
    }
     ...
     * @return list<string>
     */
    private function externalDocumentation(string $symbol): array
    {
        $descriptor = $this->externalDescriptor($symbol);
        ...
        return match ($this->externalKind($symbol)) {
            Kind::Property => ['External PHP property: ' . $this->externalQualifiedMember($descriptor, '::$')],
            Kind::Method => ['External PHP method: ' . $this->externalQualifiedMember($descriptor, '::', '()')],
            Kind::Constant => str_contains($descriptor, '#')
                ? ['External PHP constant: ' . $this->externalQualifiedMember($descriptor, '::')]
                : ['External PHP constant: ' . str_replace('/', '\\', substr($descriptor, 0, -1))],
            Kind::PBClass => ['External PHP class: ' . str_replace('/', '\\', substr($descriptor, 0, -1))],
            Kind::PBFunction => ['External PHP function: ' . str_replace('/', '\\', substr($descriptor, 0, -3)) . '()'],
            default => [],
        };
    }

    private function externalQualifiedMember(string $descriptor, string $separator, string $suffix = ''): string
    {
        if (preg_match('/^(.*)#\\$([^.#]+)\\.$/', $descriptor, $matches) === 1) {
            return str_replace('/', '\\', $matches[1]) . $separator . $matches[2];
        }

        if (preg_match('/^(.*)#([^.#()]+)\\(\\)\\.$/', $descriptor, $matches) === 1) {
            return str_replace('/', '\\', $matches[1]) . $separator . $matches[2] . $suffix;
        }
    ...
    private function externalDescriptor(string $symbol): ?string
    {
