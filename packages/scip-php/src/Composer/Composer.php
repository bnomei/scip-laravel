...
final class Composer
{
    ...
    private array $findFileCache;

    /** @var array<non-empty-string, bool> */
    private array $isFromProjectCache;

    /** @var array<non-empty-string, ?array{name: non-empty-string, version: non-empty-string}> */
    private array $pkgCache;

    /** @var array<non-empty-string, ?non-empty-string> */
    private array $constFileCache;
    ...
    public function __construct(private readonly string $projectRoot)
    {
        ...
        $this->scipPhpVendorDir = $this->resolveScipPhpVendorDir();
        ...
        $this->findFileCache = [];
        $this->isFromProjectCache = [];
        $this->pkgCache = [];
        $this->constFileCache = [];
    }

    /** @return non-empty-string */
    private function resolveScipPhpVendorDir(): string
    {
        foreach (
            [
                self::join(__DIR__, '..', '..', 'vendor'),
                self::join(__DIR__, '..', '..', '..', '..', 'vendor'),
                self::join(__DIR__, '..', '..', '..', '..'),
                // Installed as a dependency of another package, e.g. vendor/bnomei/scip-laravel/packages/scip-php/.
                self::join(__DIR__, '..', '..', '..', '..', '..', '..'),
            ] as $candidate
        ) {
            ...
            if (
                is_file(self::join($resolved, 'autoload.php'))
                && is_dir(self::join($resolved, 'jetbrains', 'phpstorm-stubs'))
            ) {
                return $resolved;
            }
        }

        throw new RuntimeException('Invalid scip-php vendor directory.');
    }
    ...
    {
        if (array_key_exists($ident, $this->findFileCache)) {
            return $this->findFileCache[$ident];
        }
        ...
        if ($f !== false && realpath($f) !== false) {
            $this->findFileCache[$ident] = realpath($f);
        ...
        if (function_exists($ident)) {
            ...
            if ($f !== false && $f !== '') {
                ...
                if (is_file($vendorFile)) {
                    $this->findFileCache[$ident] = $vendorFile;
                ...
                $this->findFileCache[$ident] = self::join($this->projectRoot, $f);
        ...
        if (str_starts_with($ident, 'Composer\\Autoload\\') && class_exists($ident)) {
            ...
            if ($f !== false && $f !== '') {
                ...
                $this->findFileCache[$ident] = str_replace($this->scipPhpVendorDir, $this->vendorDir, $f);

                return $this->findFileCache[$ident];
            }
        }

        $this->findFileCache[$ident] = $this->findConstFile($ident);
    ...
    public function pkg(string $ident): ?array
    {
        ...
        if ($this->isStub($ident)) {
            $this->pkgCache[$ident] = ['name' => 'php', 'version' => PHP_VERSION];

            return $this->pkgCache[$ident];
        }
        if ($this->isFromProject($ident)) {
            $this->pkgCache[$ident] = ['name' => $this->pkgName, 'version' => $this->pkgVersion];

            return $this->pkgCache[$ident];
        }
        $f = $this->findFile($ident);
        if ($f === null) {
            $this->pkgCache[$ident] = null;

            return null;
        }
        foreach ($this->pkgsByPaths as $path => $info) {
            if (str_starts_with($f, $path)) {
                $this->pkgCache[$ident] = $info;
    ...
    {
        if (array_key_exists($ident, $this->isFromProjectCache)) {
            return $this->isFromProjectCache[$ident];
        }

        if (str_contains($ident, 'anon-class-') || str_contains($ident, 'anon-func-')) {
            $this->isFromProjectCache[$ident] = true;

            return true;
        }
        if ($this->isStub($ident)) {
            $this->isFromProjectCache[$ident] = false;
        ...
        $this->isFromProjectCache[$ident] = !str_starts_with($f, $this->vendorDir);
    ...
    {
        if (array_key_exists($c, $this->constFileCache)) {
            return $this->constFileCache[$c];
        }

        if (!isset($this->userConsts[$c])) {
            $this->constFileCache[$c] = null;
        ...
        foreach ($files as $f) {
            ...
            if (preg_match($defineConstPattern, $content) === 1) {
                $this->constFileCache[$c] = realpath($f);
